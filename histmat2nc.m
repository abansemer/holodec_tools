function fnout = histmat2nc(options)
    ...
    arguments
        ...
        options.roundruleset {mustBeInteger} = 0
    ...
    if (options.ruleset == 0) | (options.roundruleset == 0)
        disp('Need to specify options ruleset and roundruleset')
        return
    end
    ...
    netcdf.putAtt(ncid, varid, 'Rulesets', ['All: ' num2str(options.ruleset) ';  Round: ' num2str(options.roundruleset)]);
    netcdf.putAtt(ncid, varid, 'TimeOffsetSeconds', single(options.timeoffset));

    %Add any other options to write to global here
    globaloptions = struct('zmin', single(options.zmin), 'zmax', single(options.zmax), ...
        'xmin', single(options.xmin), 'xmax', single(options.xmax), ...
        'ymin', single(options.ymin), 'ymax', single(options.ymax), ...
        'HotspotMask', uint8(options.bullseyecorrect));
    ...
    for i = 1:length(matfiles)
        ...
        if pStats.error == 0
            ...
            imagesfm_full = seconds(imagetime - startdate);
    ...
    sv = nholograms * pStats.samplevolume;
    ...
    bulk = compute_bulk_simple(concroundnorm, endbins);


    %% Write concentration and bulk to netCDF
    netcdf.putVar(ncid, netcdf.inqVarID(ncid, 'lwc_round'), bulk.lwc)
    netcdf.putVar(ncid, netcdf.inqVarID(ncid, 'mvd_round'), bulk.mvd)
    netcdf.putVar(ncid, netcdf.inqVarID(ncid, 'dmean_round'), bulk.dmean)
