function data = holoprep(options)
    % HOLOPREP  Find and save PNG files from a directory full of seq/tiff 
    % files that meet the criteria for processing.  Will use a GV/C130 
    % netCDF file with CDP (or other) data to find where cloud exists, with
    % some padding before/after cloud.
    %
    % Also generates config file properties, sequences, and job submission
    % commands.
    %
    % Options:
    ...
    if ~exist(options.ncfile, 'file')
        disp(['Flight file does not exist:' options.ncfile]);
        return
    end
    ...
    %Get flight number if not specified
    if isempty(options.flightid)
        options.flightid = data.flightnumber;
    end

    %% Construct array of times to accept holograms for (1Hz)
    goodtime = ones(1, numel(data.aircraft.time));
    ...
    %% Sequence files (CSET, SPICULE, pre-2022)
    if format == "seq"
        imagetime=[];
        brightness=[];
        seqfilenum=[];
        ...
        for i = 1:min(length(imagefiles), max2process)
           ...
           imagetime = [imagetime seqInfo.datetime];
        ...
        timeindex = floor(seconds(imagetime-data.aircraft.time(1))+1);

        %Need to clean up timeindex first to avoid negative indexing
        %Find times before/after takeoff to ignore them
        ioutofrange = find((timeindex < 1) | (timeindex > length(data.aircraft.time))); 
        timeindex(ioutofrange) = 1;   %Set all bad times to the first index
        goodtime(1) = 0;  %Make sure the first index is set to zero ('bad')

        good = find((imagetime >= data.timerange(1)) &...
            (imagetime <= data.timerange(2)) &...
            (goodtime(timeindex) == 1) &...
            (brightness > 50) &...
            (brightness < 200));
        if isempty(good)
        ...
        prefix = [options.flightid '_'];
        pngname = strings(1,numel(good));  %cell(1,numel(good));
        ...
        data.brightness = brightness(good);
        data.imagetime = imagetime(good);
        data.filename = pngname;
    end  %seq image processing
    
    %% TIFF images (ESCAPE, CAESAR, 2022-)
    if format == "tiff"
